...
import uniandes.dpoo.aerolinea.modelo.cliente.ClienteCorporativo.Cliente;
import uniandes.dpoo.aerolinea.persistencia.CentralPersistencia;
import uniandes.dpoo.aerolinea.persistencia.IPersistenciaAerolinea;
import uniandes.dpoo.aerolinea.persistencia.IPersistenciaTiquetes;
import uniandes.dpoo.aerolinea.persistencia.TipoInvalidoException;
import uniandes.dpoo.aerolinea.tiquetes.GeneradorTiquetes.Tiquete;
...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
    	for (Vuelo vuelo : vuelos) {
            if (vuelo.getRuta().equals(codigoRuta)) {
                return vuelo;
            }
        }
        return null;
    ...
    {
        // Verificar si el avión ya está ocupado en el mismo intervalo de tiempo
        for (Vuelo vuelo : vuelos) {
        ...
        // Obtener la ruta y el avión correspondientes
        ...
        // Verificar si la ruta y el avión existen
        if (ruta == null) {
        ...
        // Crear el nuevo vuelo y agregarlo a la lista de vuelos
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        if (!existeCliente(identificadorCliente)) {
            throw new Exception("El cliente no existe");
        }
        
        // Obtener el cliente y el vuelo correspondientes
        ...
        // Verificar si hay suficiente espacio en el vuelo para todos los pasajeros
        ...
        // Calcular el valor de los tiquetes según la temporada
        int valorTiquete;
        if (isTemporadaBaja(fecha)) {
            valorTiquete = vuelo.getRuta().getTarifaBaja();
        } else {
            valorTiquete = vuelo.getRuta().getTarifaAlta();
        }
        
        // Vender los tiquetes y asociarlos al cliente y al vuelo
        int valorTotal = valorTiquete * cantidad;
        for (int i = 0; i < cantidad; i++) {
            Tiquete tiquete = new Tiquete(cliente, vuelo, valorTiquete);
    ...
    /**
     * Verifica si una fecha corresponde a la temporada baja (enero a mayo y septiembre a noviembre)
     * @param fecha La fecha a verificar
     * @return true si la fecha corresponde a la temporada baja, false de lo contrario
     */
    private boolean isTemporadaBaja(String fecha) {
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente )
    {
        ...
        for (Vuelo vuelo : vuelos) {
            for (Tiquete tiquete : vuelo.getTiquetes()) {
                if (tiquete.getClienteId().equals(identificadorCliente) && !tiquete.isUsado()) {
